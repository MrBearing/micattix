...
mod integration_tests {
    use micattix::core::{Board, BoardSize, Player, Piece, GameMode};
    ...
    fn test_game_mode_players() {
        ...
        assert!(four_player_manager.session.players.contains(&Player::Second));
        assert!(four_player_manager.session.players.contains(&Player::Third));
        assert!(four_player_manager.session.players.contains(&Player::Fourth));
    }
    
    #[test]
    fn test_player_direction() {
        // 横向き移動のプレイヤー
        assert_eq!(Player::First.direction(), micattix::core::MoveDirection::Horizontal);
        assert_eq!(Player::Third.direction(), micattix::core::MoveDirection::Horizontal);
        
        // 縦向き移動のプレイヤー
        assert_eq!(Player::Second.direction(), micattix::core::MoveDirection::Vertical);
        assert_eq!(Player::Fourth.direction(), micattix::core::MoveDirection::Vertical);
    }
}